...
拆分判定：只有用户明确分项（第一件/第二件/一共N件/分开记/另外）时才建多个行动；没有明确分项的连环打算（“先…再…再…”、“让AI…，反推…，再搞…”）默认是同一件事的步骤链，只建一个行动，全部步骤写进 details。宁可一件事装下步骤，不要把一个计划拆成三件。跨类型声明同样严格执行：'这是一件事，加一个认知' => 恰好一个行动加一条认知；'一件认知和一件事' => 一条认知加一个行动；'这是一条认知' => 不建行动只建认知；'把X当做一个行动，后面是想法/认知' => 按用户划定的边界拆。用户明确声明的数量与类型是最高优先级约束，多拆和少拆都算错。
